...
### 3. 全局设置页面
- **答题连接**：为所有账户配置共享的基础地址、模型、API Key、超时、重试和答题并发
- **运行限制**：设置整套应用同时运行的账户数量上限
- **账户工作台**：仅在对应账户页面设置课程选择和学习参数，不重复提供通知/OCR 控件

### 4. 后端兼容配置
- **OCR**：Web 任务运行链仍可读取后端保存的 `ocr_config`，因此保留 OCR 兼容能力；当前 Web UI 不提供账户级 OCR 配置控件
- **通知**：账户 `notification_config` 仅为 legacy/deprecated 数据库/API 兼容字段保留，当前 Web 执行路径不可达，不会自动推送；完成/错误通知请使用 CLI 配置
...
6. **全局设置**（可选） - 在侧边栏打开“全局设置”，配置共享答题连接和运行限制
7. **开始学习** - 点击"开始学习"按钮
8. **查看进度** - 实时监控学习进度和日志
9. **完成任务** - 等待所有任务完成；Web 不自动推送通知
...
### OCR 与通知的边界
- **OCR**：后端仍兼容从保存的 `ocr_config` 读取 OCR 服务；当前 Web UI 没有账户级配置控件
- **通知**：`notification_config` 仅作为 legacy/deprecated 数据库/API 兼容字段保留，Web 执行路径不可达
- **CLI 通知**：需要完成或错误通知时，在 CLI 的 `[notify]` 配置服务 URL、Token、Chat ID 等字段
