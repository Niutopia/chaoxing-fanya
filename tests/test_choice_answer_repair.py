...
from api.answer import AI, CacheDAO, SiliconFlow, Tiku, question_cache_key
...
def test_invalid_first_answer_gets_one_strict_repair_request_and_maps(monkeypatch):
    ...
    assert provider._cache.values[question_cache_key(question)] == "B"
...
def test_repair_accepts_only_pure_label_when_explanation_conflicts(monkeypatch):
    ...
    assert question_cache_key(question) not in provider._cache.values
...
def test_failed_choice_cache_is_removed_so_next_work_queries_provider_again(
    ...
    assert first_outcome is StudyResult.SKIPPED
    assert question_cache_key(question) not in provider._cache.values

    second_outcome, second_session = _run_ai_work(monkeypatch, provider, question)

    assert second_outcome is StudyResult.SKIPPED
...
def test_siliconflow_repair_makes_one_request_even_when_max_retries_is_zero(
    ...
    cache = _MemoryCache()
    question = _choice_question()
    cache.values[question_cache_key(question)] = "not one of the options"
    ...
    assert question["answerSourceq1"] == "cover"
    assert session.posts[0]["answerq1"] == "B"
    assert cache.values[question_cache_key(question)] == "B"
...
def test_non_ai_true_cache_hit_stays_unknown_and_save_only(monkeypatch):
    ...
    provider._cache.values[question_cache_key(question)] = "true"
    ...
    assert session.posts[0]["pyFlag"] == "1"
    assert session.posts[0]["answerq1"] == ""
    assert provider._cache.values[question_cache_key(question)] == "true"
...
def test_unknown_judgement_cache_is_cleared_and_second_run_queries_provider(
    ...
    question["answerField"]["answertypeq1"] = "1"
    cache = _MemoryCache()
    cache.values[question_cache_key(question)] = "provider explanation"
    ...
    assert question_cache_key(question) not in cache.values
    ...
    assert cache.values[question_cache_key(question)] == "true"

    third_outcome, third_session = _run_ai_work(monkeypatch, provider, question)

    assert third_outcome is StudyResult.SKIPPED
    assert len(client.calls) == 1
...
def test_partial_completion_is_not_cached_and_canonical_fill_hits_on_next_run(
    ...
    assert cache.values[question_cache_key(question)] == "alpha\nbeta\ngamma"
...
def test_partial_completion_cache_is_cleared_before_retrying_provider(
    ...
    cache = _MemoryCache()
    cache.values[question_cache_key(question)] = "alpha\n\ngamma"
...
def test_stale_repair_cleanup_does_not_delete_concurrent_canonical(
    ...
    question = _choice_question()
    cache.add_cache(question_cache_key(question), "bad cached answer")
    ...
    cache.replace_cache(question_cache_key(question), "B")
    ...
    assert session.posts[0]["pyFlag"] == "1"
    assert session.posts[0]["answerq1"] == ""
    assert cache.get_cache(question_cache_key(question)) == "B"
...
def test_legacy_cache_cleanup_skips_non_atomic_expected_fallback_during_race(
    ...
    cache.values[question_cache_key(question)] = "bad cached answer"
    ...
    def write_canonical_between_get_and_remove():
        ...
        cache.values[question_cache_key(question)] = "B"
    ...
    assert not worker.is_alive()
    assert result["value"][0] is StudyResult.SKIPPED
    assert question["answerSourceq1"] == "uncovered"
    assert cache.values[question_cache_key(question)] == "B"
...
def test_legacy_add_only_cache_skips_expected_cleanup_without_overwrite(
    ...
    assert outcome is StudyResult.SKIPPED
    assert cache.values[question_cache_key(question)] == "bad cached answer"
