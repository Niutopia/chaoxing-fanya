"""Credential redaction and readable log persistence regressions."""
...
def test_historical_progress_redacts_known_credentials_without_erasing_context():
    assert (
        sanitize_log_message("progress LIVESECRET", secrets=("LIVESECRET",), historical=True)
        == "progress [redacted]"
    )
...
def test_historical_unknown_progress_remains_readable_with_uppercase_prefix():
    assert (
        sanitize_log_message("PROGRESS waiting for prerequisite", historical=True)
        == "PROGRESS waiting for prerequisite"
    )
...
def test_load_web_tasks_returns_clean_history_without_writing(tmp_path):
    ...
    assert loaded[0]["logs"][0]["message"] == "unrecognised old free text"
    with sqlite3.connect(store.db_path) as connection:
...
def test_manager_owns_best_effort_sqlite_historical_writeback(tmp_path):
    ...
    assert manager.get_logs("task-migrate").items[0].message == "unrecognised legacy question text"
    with sqlite3.connect(store.db_path) as connection:
        row = connection.execute(
            "SELECT level, message FROM web_task_logs WHERE task_id = 'task-migrate'"
        ).fetchone()
    assert row == ("info", "unrecognised legacy question text")
                ...
                "logs": [
                    {
                        "sequence": 1,
                        "level": "info",
                        "message": "连接失败，password=legacy-value",
                        "timestamp": 1,
                    }
                ],
            }
...
def test_historical_migration_clears_dirty_only_after_retry_succeeds():
    ...
    assert page.items[0].message == "连接失败，password=[redacted]"
...
def test_worker_error_keeps_failure_reason_and_redacts_credentials():
    def fail(_context):
        raise RuntimeError("连接失败，password=legacy-value")
    ...
    assert manager.get_snapshot(task.id).error == "连接失败，password=[redacted]"
