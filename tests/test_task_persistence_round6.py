...
def test_legacy_terminal_error_cleaning_marks_dirty_and_retries_writeback():
            ...
            return [
                {
                    "snapshot": {
                        ...
                        "account_id": "round6-account",
                        "state": "failed",
                        "error": "连接失败，password=legacy-value",
                    },
                    "details": {},
                    "logs": [],
                }
    ...
    assert record.snapshot.error == "连接失败，password=[redacted]"
    assert record.record_dirty is True
    assert persistence.saved[-1][0]["error"] == "连接失败，password=[redacted]"

    persistence.allow = True
    assert manager.get_snapshot(record.snapshot.id).error == "连接失败，password=[redacted]"
...
def test_sqlite_loader_marks_legacy_terminal_error_dirty_without_writing(tmp_path):
    ...
    assert loaded[0]["snapshot"]["error"] == "连接失败，password=[redacted]"
    ...
    with sqlite3.connect(store.db_path) as connection:
        ...
        ).fetchone()[0])["error"] == "连接失败，password=legacy-value"
