...
def _json_compatible(value: Any) -> Any:
    ...
    return sanitize_monitor_value(value)
...
def _redact(value: Any, secrets: tuple[str, ...], *, key: Any = None) -> Any:
    ...
    if key_name in _SENSITIVE_KEYS or any(
        ...
        return "[redacted]"
    return sanitize_monitor_value(value, secrets=secrets)
...
class _TaskRuntime:
    ...
    # Set when log persistence failed or restored credentials/levels need
    # writeback. API callers see the readable, redacted entries meanwhile.
...
class TaskManager:
    ...
    def _details_mapping(details: TaskDetails) -> dict[str, Any]:
        ...
        return {
            ...
            "answer_report": _json_compatible(_copy(details.answer_report)),
        }
    ...
    def _restore_details(value: Any) -> TaskDetails:
        ...
        return TaskDetails(
            ...
            answer_report=_copy(dict(value.get("answer_report", {}))) if isinstance(value.get("answer_report"), Mapping) else {},
        )

    def _decode_logs(
        self, value: Any, *, ordered: bool = False
    ) -> tuple[deque[TaskLogEntry], int, bool]:
        """Restore readable logs and retry changed credentials/metadata."""
        ...
        def decode_one(raw_entry: Any) -> None:
            ...
            try:
                ...
                # Compare to the stored text: comparing two sanitized forms
                # would hide changes and skip their durable writeback.
                if not isinstance(message, str) or cleaned_message != message:
                    dirty = True
                if not isinstance(level, str) or sanitize_log_level(level) != level:
                    dirty = True
            except BaseException:
                return
            entry = TaskLogEntry(
    ...
    def _restore_record(
        ...
        *,
        ...
        if details.answer_report.get("status") == "running":
            details = replace(details, answer_report={**details.answer_report, "status": "failed", "error": "服务重启中断了判分读取，请重新刷新。"})
            record_dirty = True
        if interrupted:
    ...
    def begin_answer_report(self, task_id: str) -> float | None:
        with self._lock:
            record = self._lookup(task_id)
            if record.snapshot.state in {"running", "stopping"}:
                raise ValueError("请等本次学习结束后刷新判分")
            previous = record.details.answer_report
            if previous.get("status") == "running":
                return None
            started_at = time.time()
            report = {**previous, "status": "running", "started_at": started_at,
                      "error": None, "completed_chapters": 0, "total_chapters": 0}
            record.details = replace(record.details, answer_report=report)
            self._persist_record_locked(record)
            return started_at

    def update_answer_report(self, task_id: str, started_at: float, values: Mapping) -> bool:
        with self._lock:
            record = self._lookup(task_id)
            current = record.details.answer_report
            if current.get("started_at") != started_at or current.get("status") != "running":
                return False
            report = {**current, **sanitize_monitor_value(dict(values))}
            record.details = replace(record.details, answer_report=report)
            if report.get("status") in {"completed", "partial"}:
                grade_counts = {**report.get("counts", {}), "answer_checked_at": report.get("checked_at", 0),
                                "answer_scope_current_courses": 1}
                record.details = replace(record.details, counts={**record.details.counts, **grade_counts})
                record.snapshot = replace(record.snapshot, stats={**record.snapshot.stats, **grade_counts})
            self._persist_record_locked(record)
            return True
