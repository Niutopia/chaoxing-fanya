...
function activeTaskFor(accountId, task, tasks) {
  const candidate = task || (Array.isArray(tasks)
    ? tasks.find((item) => (
      String(item?.account_id ?? item?.accountId) === String(accountId)
      && ACTIVE_TASK_STATES.has(item.state)
    ))
    : null)
...
function LaunchPage({
  ...
  const loadPage = useCallback(async ({ refresh = false } = {}) => {
    ...
    // Attach rejection handlers now: the course request may take much longer
    // or be aborted during navigation before we reach the settings below.
    const settingsResults = refresh ? null : Promise.allSettled([
      preferencesPromise,
      connectionPromise,
    ])
    ...
    if (!refresh) {
      const [preferencesResult, connectionResult] = await settingsResults
      if (!isCurrentRequest()) return
